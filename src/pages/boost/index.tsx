...
export default function Boostv3() {
  ...
  const totalActiveVesting = CurrencyAmount.fromRawAmount(token, earnedBalances?.total?.toString() || '0')
  const totalActiveVestingPenalty = totalActiveVesting?.divide(2)?.toExact()?.toBigNumber(token.decimals)
  const totalCompletedVesting = CurrencyAmount.fromRawAmount(
    token,
    withdrawableBalance?.amountWithoutPenalty?.toString() || '0'
  )
  const rewards = useRewardsBalance()
  const pendingFarmsRewards = useFarmsReward()
  ...
  const insufficientFunds =
    !balance?.greaterThan(ZERO) || (parsedAmount?.greaterThan(ZERO) && parsedAmount?.greaterThan(balance))
  const inputError = insufficientFunds
          ...
          <div className="flex flex-col w-full md:w-[40%] p-4 justify-between border rounded-2xl border-light-stroke/50 dark:border-dark-stroke/50 bg-light-bg dark:bg-dark-bg">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <StatButton
                ...
                value={`${
                  !veEmosSupply || !emosSupply ? '-' : ((Number(veEmosSupply) / Number(emosSupply)) * 4).toFixed(3)
                } ${i18n._(t`Years`)}`}
              />
            </div>
          </div>
