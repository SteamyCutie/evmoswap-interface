...
const IfoCardData: React.FC<Props> = ( { ifo } ) => {
    ...
    )} (${basicTotalLPCommitted} EMO) (${basicTotalCommittedPercent === 'NaN' ? 0 : basicTotalCommittedPercent}%)`
    const unLimitedTotalCommitted = `~$${formatNumber( unLimitedTotalLPCommittedInUSD.toNumber() )} (${unLimitedTotalCommittedPercent === 'NaN' ? 0 : unLimitedTotalCommittedPercent
        }%)`
    ...
    const basicYourCommitted = `~$${formatNumber( basicYourLPCommittedInUSD.toNumber() )} (${basicYourLPCommitted} EMO)`
    ...
    return (
        <div className="grid my-8 border-[2px] border-b-[3px] border-[#383241] text-[#f4eeff] rounded-[32px] animate-fade">
            <div
                className={ `flex items-center justify-end w-full h-[112px] bg-no-repeat bg-cover bg-center rounded-[30px] ${isOpen ? 'rounded-b-none' : 'rounded-b-[30px]'
                    } p-6 ` }
                        ...
                        <div className="grid w-full grid-cols-1 gap-4 md:grid-cols-2">
                            <div className="space-y-6 rounded-3xl bg-light-secondary dark:bg-dark-secondary">
                                <div className="flex flex-row justify-between px-6 py-4 items-center rounded-t-3xl item-center bg-gradient-to-b from-[#6c4999] to-[#6d4db2]">
                                    ...
                                    <div className="bg-gray-700/80 text-yellow h-[24px] pr-3 whitespace-nowrap inline-flex rounded-sm pl-3 font-bold text-sm items-center justify-center">
                                        Finished
                                    </div>
                                </div>
