...
const Background: FC<Background> = ( { variant } ) => {
    if ( !variant ) return <div className="absolute inset-0 bg-light dark:bg-dark/30" />

    const { type, url } = VARIANTS[ variant ]

    if ( type === ImageType.REPEAT ) {
        return (
            <div className={ classNames( 'absolute inset-0 flex flex-col items-center bg-light dark:bg-dark/30' ) }>
