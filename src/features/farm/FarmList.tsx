    ...
    <>
      <div className={classNames(TABLE_WRAPPER_DIV_CLASSNAME)}>
        <div className="hidden md:flex md:w-full bg-light-secondary dark:bg-dark-secondary rounded-md font-medium md:px-6">
          ...
          >
            <Typography weight={500}>{i18n._(t`Pool`)}</Typography>
            <SortIcon id={sortConfig.key} direction={sortConfig.direction} active={sortConfig.key === 'symbol'} />
          </div>
          <div className={classNames('flex gap-1 items-center cursor-pointer', TABLE_TR_TH_CLASSNAME(1, 6))}>
            <Typography weight={500}>{i18n._(t`Rewards`)}</Typography>
          </div>
          <div
            className={classNames('flex gap-1 items-center cursor-pointer', TABLE_TR_TH_CLASSNAME(2, 6))}
            onClick={() => requestSort('tvl')}
          >
            <Typography weight={500}>{i18n._(t`TVL`)}</Typography>
          ...
          <div
            ...
            // onClick={() => requestSort('tvl')}
          >
            <Typography weight={500}>{i18n._(t`Deposits`)}</Typography>
            {/* <SortIcon id={sortConfig.key} direction={sortConfig.direction} active={sortConfig.key === 'tvl'} /> */}
          </div>
          <div
          ...
          >
            <Typography weight={500}>{i18n._(t`Multiplier`)}</Typography>
          ...
          >
            <Typography weight={500}>{i18n._(t`APR -> Boost APR`)}</Typography>
          ...
          >
            {items.slice(0, numDisplayed).map((farm, index) => (
              <React.Fragment key={index}>
                <FarmListItem key={index} farm={farm} onClick={() => handleOpen(farm)} />
