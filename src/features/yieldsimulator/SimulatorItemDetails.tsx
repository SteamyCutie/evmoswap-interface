...
import { useUserInfo } from '../farm/hooks'
...
const SimulatorItemDetails = ({ farm, veEmos, handleBoost }) => {
  ...
  const [share, setShare] = useState(Number(stakedAmount?.toSignificant(6, undefined, 4)) / farm.tvl)
  const [earning, setEarning] = useState((Number(stakedAmount?.toSignificant(6, undefined, 4)) * (apy / 100)) / 365)

  const handleInput = (val) => {
    setDepositValue(val)
    const value = Number(stakedAmount?.toSignificant(6, undefined, 4)) + Number(depositValue)
          ...
          <div className="flex items-center justify-between w-full md:w-5/12">
            <div className="text-white">
              ...
              <div className="text-lg font-bold md:text-xl">
                {formatNumberScale(stakedAmount?.toSignificant(6, undefined, 4) ?? 0, false, 4)}
              </div>
            </div>
