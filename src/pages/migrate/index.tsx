...
import MetamaskError from 'app/features/migration/MetamaskError'
...
const MigrateButtons = ( { state, exchange }: { state: MigrateState; exchange: string | undefined } ) => {
    ...
    const emoRollContract = useEvmoRollContract( state.selectedLPToken?.dex )
    // console.log(
    //   'emoRollContract address',
    //   emoRollContract?.address,
    //   state.selectedLPToken?.balance,
    //   state.selectedLPToken?.version
    // )

    const [ approval, approve ] = useApproveCallback( state.selectedLPToken?.balance, emoRollContract?.address )
