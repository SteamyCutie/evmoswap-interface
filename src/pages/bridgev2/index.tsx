      ...
      <div className="w-full m-auto mt-8 space-y-8">
        ...
        <div className="grid w-full gap-4">
          {BRIDGE_DATA.map((item, idx) => (
            <div
              key={idx}
              className="flex items-center w-full bg-dark-900 text-primary px-12 py-8 rounded-2xl space-x-8"
            >
              ...
              <div className="grid w-1/5">
                <a href={item.active && item.link} target="_blank" rel="noreferrer" aria-disabled={!item.active}>
                  <Button color="gradient" className="w-full" variant="filled" disabled={!item.active}>
                    {i18n._(t`Bridge`)}
                  </Button>
                </a>
              </div>
